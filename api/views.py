...
from api.models import Difference, Query, Result, Scrape, Status
from api.scraper import Scraper
from api.serializers import (DifferenceSerializer, DifferenceSerializerArray,
                             QuerySerializer, ResultDetailSerializer,
                             ResultListSerializer, ScrapeSerializer)
...
class QueryViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer: QuerySerializer) -> None:
        ...
        perform_scrape.delay(scrape.id)
...
@method_decorator(
    name="list", decorator=swagger_auto_schema(manual_parameters=[query_id])
)
class ScrapeViewSet(
...
@method_decorator(
    name="list", decorator=swagger_auto_schema(manual_parameters=[scrape_id])
)
...
def get_scrapes_from_params(request: Request) -> Tuple[Scrape, Scrape]:
    ...
    try:
        ...
        raise ValidationError(
            {"status": "error", "message": "Invalid scrape1_id"},
    ...
    try:
        ...
        raise ValidationError(
            {"status": "error", "message": "Invalid scrape2_id"},
            status.HTTP_400_BAD_REQUEST,
        )

    if scrape1.query != scrape2.query:
        raise ValidationError(
            {"status": "error", "message": "Both scrapes belong to different queries"},
            status.HTTP_400_BAD_REQUEST,
        )
...
def get_difference(id: int, as_arrays: bool = False) -> Response:
    try:
        ...
        return Response(
            {"status": "error", "message": "Difference with given id does not exist."},
...
def get_difference_urls(request: Request) -> Response:
    ...
    data = {
        "scrape1": ScrapeSerializer(scrape1).data,
        "scrape2": ScrapeSerializer(scrape2).data,
        "urls_added": list(urls_added),
        "urls_removed": list(urls1 - urls2),
        "unique_urls_added": [list(unique_urls_added)],
    }
    return Response(data, status=status.HTTP_200_OK)


@swagger_auto_schema(method="get", manual_parameters=difference_views_parameters)
@api_view(("GET",))
@renderer_classes((JSONRenderer,))
def get_results_with_difference(request):
    ...
    data = {
        ...
        "results": [
            {
                "id": difference.id,
                "page_link": difference.result2.page_link,
                "page_title": difference.result2.page_title,
            }
...
@swagger_auto_schema(method="get", responses={200: DifferenceSerializer()})
@api_view(("GET",))
@renderer_classes((JSONRenderer,))
...
@swagger_auto_schema(method="get", responses={200: DifferenceSerializerArray()})
@api_view(("GET",))
@renderer_classes((JSONRenderer,))
