...
from api.models import Difference, Query, Result, Scrape
...
class ScrapeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Scrape
        fields = "__all__"
        read_only_fields = ("query", "started_at", "completed_at", "status", "log")

    def create(self, validated_data):
        scrape = Scrape(query=validated_data.pop("query_id"), **validated_data)
        scrape.save()
        return scrape
